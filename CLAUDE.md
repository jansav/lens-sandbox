...
- **Concepts:** approvals drive policy authoring; a per-directory decisions file (`lns-local-mixin.yaml`, a `kind: mixin` document auto-created empty; a destination no rule decides is asked about) holds the network rules, while which connectors a project connected is per machine and lives in the grant sidecar, connector declarations live in the installed set on this machine (`~/.lns/connectors.yaml`), never inside `lns`, and per-machine credential values are stored separately again so secrets aren't committed; `Vz` on macOS / `KVM` on Linux is the only runtime; real secrets stay outside the workload via credential-shaped placeholders.
...
Real shipped ids (`github`, `openai`, `anthropic`, `linear`, `telegram`, `gitlab`, …) belong nowhere in **tests or code**. Nothing ships inside `lns`: a connector is declared on the machine that installs it, so there is no in-tree catalog for a test to pin. *User docs* are the exception, and only for illustration: a recognizable host as a destination example, a real service in a `lns connector` walkthrough the reader would actually type, or a catalog-file example that shows a real declaration.

The reason a synthetic fixture is the only durable one: a test pinned to a real id churns every time that connector changes, and re-pointing `github` → `openai` is not a fix, because `openai` moves too. When a test names a real provider or connector as its example, replace it with a synthetic fixture.
