...
/// Append a sandbox-run event to the audit chain, pinning the resolved digest (not just the mutable tag) and the shipped-policy hash; a recording failure is logged, never fatal to the launch.
